...
import {
    GPKickAdapterContract,
    ProposalProcessed,
    ProposalCreated
} from "../generated/GPKickAdapterContract/GPKickAdapterContract"
import { VintageRiaserOutProposal } from "../generated/schema"

export function handleProposalCreated(event: ProposalCreated): void {
    // Entities can be loaded from the store using a string ID; this ID
    // needs to be unique across all entities of the same type
    let entity = VintageRiaserOutProposal.load(event.params.proposalId.toHex())

    // Entities only exist after they have been saved to the store;
    // `null` checks allow to create entities on demand
    if (!entity) {
        entity = new VintageRiaserOutProposal(event.params.proposalId.toHex())
    ...
    entity.daoAddr = event.params.daoAddr;
    entity.proposalId = event.params.proposalId;
    entity.raiserAddress = event.params.gpToKick;
